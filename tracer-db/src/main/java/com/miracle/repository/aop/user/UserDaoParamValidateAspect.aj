package com.miracle.repository.aop.user;
...
/**
 * Description: 对{@link com.miracle.repository.dao.user.UserDao}进行验证的切面
...
public class UserDaoParamValidateAspect {
    ...
    @Pointcut("execution(public com.miracle.data.po.result.user.UserPersisitenceResult com.miracle.repository.dao.user.UserDao.*(..))")
    public void validate() { }

    /**
     * 为insert方法校验{@link UserPO}的合法性
     * @param joinPoint 连接点
     * @return 操作结果
     */
    @Around("execution(public com.miracle.data.po.result.user.UserPersisitenceResult " +
            "com.miracle.repository.dao.user.UserDao.insert(com.miracle.data.po.data.user.UserPO))")
    public UserPersisitenceResult validateUserWhenInsert(ProceedingJoinPoint joinPoint) {
    ...
    /**
     * 为update方法校验{@link UserPO}的合法性
     * @param joinPoint 连接点
     * @return 操作结果
     */
    @Around("execution(public com.miracle.data.po.result.user.UserPersisitenceResult " +
            "com.miracle.repository.dao.user.UserDao.update(com.miracle.data.po.data.user.UserPO))")
    public UserPersisitenceResult validateUserWhenUpdate(ProceedingJoinPoint joinPoint) {
        final UserPO user = AspectUtils.getFirstArgFromJoinPoint(joinPoint);
        return AppliableValidator.of(user)
